...
def submit_report(payload: ReportCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
  ...
  mentors = db.query(User).filter(User.role.in_(["mentor", "admin"])).all()
  db.add_all([Notification(user_id=user.id, message=f"{current_user.name} submitted a report") for user in mentors])
